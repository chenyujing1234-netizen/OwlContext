...
const ScreenMonitor: React.FC = () => {
  ...
  // Auto-start recording if token is configured (OwlContext custom feature)
  useEffect(() => {
    const checkTokenAndAutoStart = async () => {
      // Skip if already attempted or already monitoring
      if (autoStartAttemptedRef.current || isMonitoring) {
        return
      }
      ...
      try {
        ...
        const apiKey = modelInfo?.config?.apiKey

        if (!apiKey || apiKey.trim() === '') {
          logger.info('Token not configured, skipping auto-start')
          autoStartAttemptedRef.current = true
          return
        }

        logger.info('Token configured, checking conditions for auto-start')

        // Check if all conditions are met:
        // 1. Has permission
        // 2. Is today
        // 3. Has configured sources (screen or window)
        // 4. Can record (within recording hours if enabled)
        if (!hasPermission) {
          logger.info('No permission, skipping auto-start')
          autoStartAttemptedRef.current = true
          return
        }

        if (!isToday) {
          logger.info('Not today, skipping auto-start')
          autoStartAttemptedRef.current = true
          return
        }

        // Check if sources are configured
        if (settingSources.state !== 'hasData') {
          // Wait a bit for sources to load, then retry
          setTimeout(() => {
            if (!autoStartAttemptedRef.current && !isMonitoring) {
              checkTokenAndAutoStart()
            }
          }, 2000)
          return
        }
        ...
        const hasConfiguredSources =
          (settingScreenSources && settingScreenSources.length > 0) ||
          (settingWindowSources && settingWindowSources.length > 0) ||
          (screenAllSources && screenAllSources.length > 0)

        if (!hasConfiguredSources) {
          logger.info('No configured sources, skipping auto-start')
          autoStartAttemptedRef.current = true
          return
        }

        // Check if can record now
        const recordCheck = await checkCanRecord()
        if (!recordCheck.canRecord) {
          logger.info('Cannot record now (outside recording hours), skipping auto-start')
          autoStartAttemptedRef.current = true
          return
        }

        // All conditions met, start monitoring
        logger.info('All conditions met, auto-starting recording')
        autoStartAttemptedRef.current = true
        startMonitoring()
      } catch (error) {
        logger.error('Failed to check token or auto-start recording', { error })
        autoStartAttemptedRef.current = true
      }
    }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hasPermission, isToday, isMonitoring, settingSources, settingScreenSources, settingWindowSources, screenAllSources])

  // Reset auto-start flag when date changes or monitoring stops
  useEffect(() => {
    if (!isMonitoring) {
      autoStartAttemptedRef.current = false
    }
  }, [isMonitoring, currentDate])
