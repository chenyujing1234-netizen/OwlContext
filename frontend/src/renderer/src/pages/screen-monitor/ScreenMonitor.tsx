...
const ScreenMonitor: React.FC = () => {
  ...
  // Auto-start recording if token is configured
  useEffect(() => {
    const checkTokenAndAutoStart = async () => {
      ...
      try {
        ...
        const apiKey = modelInfoResponse?.data?.config?.apiKey
        ...
        const settingSourcesState = settingSources.state
        if (settingSourcesState !== 'hasData') {
