...
import { app, BrowserWindow, Menu, nativeImage, NativeImage, shell, Tray } from 'electron'
...
export class TrayService {
  ...
  create(): void {
    ...
    try {
      ...
      // Set tooltip
      this.tray.setToolTip('OwlContext')
  ...
  private buildContextMenu(): Menu {
    ...
    const menuTemplate: Electron.MenuItemConstructorOptions[] = [
      ...
      },
      {
        label: '应用管理',
        click: () => {
          shell.openExternal('http://127.0.0.1:1733/contexts').catch((error) => {
  ...
  updateRecordingStatus(isRecording: boolean): void {
    ...
    if (this.tray) {
      ...
      // Update tooltip
      const tooltip = isRecording ? 'OwlContext - 录制中' : 'OwlContext - 已暂停'
