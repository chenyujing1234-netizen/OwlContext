...
async function findRunningBackendPort(startPort: number = 1733, maxAttempts: number = 20) {
  for (let port = startPort; port < startPort + maxAttempts; port++) {
    ...
    try {
      ...
      logToBackendFile(`Port ${port} is occupied but not a healthy OwlContext backend: ${error.message}`)
    }
  }
  return null
}
...
async function startBackendServer(mainWindow: BrowserWindow) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Also check for app.asar.unpacked directory
      const unpackedPath = path.join(actualResourcesPath, 'app.asar.unpacked')
      if (fs.existsSync(unpackedPath)) {
        actualResourcesPath = unpackedPath
        logToBackendFile(`Found app.asar.unpacked directory, using: ${actualResourcesPath}`)
      }
      ...
      const possiblePaths = [
        ...
        path.join(actualResourcesPath, 'resources', 'backend', executableName),
        path.join(actualResourcesPath, 'resources', 'backend', 'dist', executableName),
