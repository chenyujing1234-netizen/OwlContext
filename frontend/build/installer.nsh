; Custom NSIS script for OwlContext installer
...
!macro customInit
  ...
  ReadRegStr $0 HKCU "Software\OwlContext" "InstallDirectory"
  ${If} $0 != ""
    StrCpy $INSTDIR $0
  ${EndIf}

  ; Try to read existing data directory from registry
  ReadRegStr $DataDir HKCU "Software\OwlContext" "DataDirectory"
  ${If} $DataDir == ""
    ; Set default to AppData\Local\OwlContext
    StrCpy $DataDir "$LOCALAPPDATA\OwlContext"
  ${EndIf}
!macroend
...
Function DataDirectoryPage
  ...
  ${NSD_CreateLabel} 0 0 100% 24u "Select where OwlContext will store application data (databases, files, cache). The directory requires write permissions."
  Pop $DataDirLabel
...
Function DataDirectoryLeave
  ...
  WriteRegStr HKCU "Software\OwlContext" "DataDirectory" "$DataDir"
FunctionEnd
